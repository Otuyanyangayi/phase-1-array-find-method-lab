...
const record = [
  ...
  // more practice 
  function superbowlLoose(record){
      const result = record.find(record => record.result === "L")
      return !!result ? result.year : undefined;
  }
  superbowlLoose(record)
  function notAttend (record){
      const result = record.find(record => record.year == "N/A")
      return !! result ? result.year : undefined
  }